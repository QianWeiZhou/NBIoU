...
def train(Dataset, Network):
    ## dataset
    save_dir = '/home/work3/PCRNet/PCRNet_RGB/out/train_loss'
    img_dir = save_dir + '/img'
    if not os.path.isdir(save_dir):
        os.mkdir(save_dir)
    if not os.path.isdir(img_dir):
        os.mkdir(img_dir)
    cfg = Dataset.Config(datapath='/home/work3/PCRNet/PCRNet_RGB/data/DUTS-TR', savepath=save_dir, mode='train', batch=16, lr=0.03, momen=0.9,
                         decay=5e-4, epoch=60)
...
def __init__(self, Dataset, Network, Path):
    ## dataset
    self.save_dir ='.PCRNet_RGB/out/mae'
    self.epoch = 60
    ...
    for epoch in range(cfg.epoch):
        ...
        with torch.no_grad:
            if __name__=='__main__':
                for set in ['PASCAL-S']:#'PASCAL-S','ECSSD','SOD','HKU','DUTS-TE','DUT-OMRON'
                    path  = '.PCRNet_RGB/data/testset/'+set
                    #path = '../data/DUTS-TR'
                    t = Test(dataset, LDF, path)
                    t.save() #不同数据集epoch的mae

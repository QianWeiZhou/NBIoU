...
class Data(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = cv2.imread(self.cfg.datapath + '/images/' + name + '.jpg')
        if image is None:
            image = cv2.imread(self.cfg.datapath + '/images/' + name + '.png')
        image = image[:, :, ::-1].astype(np.float32)
        if self.cfg.mode == 'train':
            mask = cv2.imread(self.cfg.datapath + '/GT/' + name + '.png', 0).astype(np.float32)
            body = cv2.imread(self.cfg.datapath + '/body-origin/' + name + '.png', 0).astype(np.float32)#body-origin

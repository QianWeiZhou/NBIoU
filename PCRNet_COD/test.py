...
class Test(object):
    def __init__(self, Dataset, Network, Path):
        ## dataset
        self.save_dir ='./out/COD_0104_220/'
        self.epoch = 220
    ...
    def save(self):
        ...
        with torch.no_grad():
            for image, (H, W), name in self.loader:
                ...
                d1,b1,out = self.net(image, shape)
                #out  = out1
                ...
                #
                # pred = torch.sigmoid(b1[0,0]).cpu().numpy()*255
                ...
                # cv2.imwrite(head+'/'+name[0]+'.png', np.round(pred))
                #
                # pred = torch.sigmoid(d1[0,0]).cpu().numpy()*255
...
if __name__=='__main__':
    for set in ['COD','CAMO','CHAMELEON','NC4K']:#'PASCAL-S','ECSSD','SOD','HKU','DUTS-TE','DUT-OMRON' ,'COD','CAMO','CHAMELEON','NC4K'
        #path  = '../data/testset/'+set
        #path = '../data/DUTS-TR'
        path = '../data/COD/Test/' + set
        #path = '../data/' + set
